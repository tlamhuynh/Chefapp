...
export function CreativeAgent({
  ...
  const [processingConversations, setProcessingConversations] = useState<
    Set<string>
  >(new Set());
  const [error, setError] = useState<string | null>(null);

  const isProcessing = activeConversationId
    ? processingConversations.has(activeConversationId)
    : false;
  ...
  const handleClearAllHistory = async () => {
    ...
    setConfirmModal({
      ...
      message:
        "Bạn có chắc chắn muốn xoá tất cả bản thảo sáng tạo không? (Hành động này không thể hoàn tác)",
      onConfirm: async () => {
        try {
          const q = query(
            collection(db, "creation_history"),
  ...
  const handleSend = async (text: string = input) => {
    ...
    if (!currentConvId) {
      ...
      try {
        await updateDoc(
          doc(db, "creative_conversations", currentConvId),
          updates
        );
      } catch (err) {
    ...
    try {
      ...
      const aiMsg = {
        conversationId: currentConvId,
        text:
          aiResult.text || "Xin lỗi, tôi gặp chút trục trặc. Bạn thử lại nhé!",
        sender: "ai",
        userId: auth.currentUser.uid,
        timestamp: serverTimestamp(),
      ...
      <aside className="hidden lg:flex w-16 flex-col bg-white border-r border-stone-100 z-30 shrink-0">
        ...
        <div className="p-4 border-t border-stone-50 flex flex-col items-center gap-4">
          <button
            onClick={() =>
              updatePreference(
                ...
                !preferences.showInternalThoughts
              )
            }
            className={cn(
          ...
          <button
            ...
            className="w-10 h-10 rounded-xl flex items-center justify-center text-stone-300 hover:text-stone-900 transition-all"
          >
          ...
          >
            ...
            <div className="flex-1 overflow-y-auto px-3 space-y-6 no-scrollbar pb-6 mt-1">
              <div className="space-y-2">
                <p className="text-[9px] font-bold text-stone-400 uppercase tracking-[0.2em] px-1">
                  Gần đây
                </p>
                ...
                {filteredConversations?.map((conv) => (
                      <div key={conv?.id || Math.random()} className="group relative">
                        <button
                          onClick={() => {
                            if (conv?.id) {
                          ...
                          <div
                            className={cn(
                              ...
                              activeConversationId === conv?.id
                                ...
                                : "bg-transparent text-stone-400 group-hover:bg-stone-200 group-hover:text-stone-700"
                            )}
                          >
              ...
              <div className="flex items-center gap-2 group pointer-events-auto">
                <h1 className="font-display font-semibold text-xl text-stone-900">
                  {conversations.find((c) => c.id === activeConversationId)
                    ?.title || "GemAgent Sáng tạo"}
                </h1>
              </div>
              <div className="flex items-center gap-1 mt-1">
                <span className="text-[9px] font-bold uppercase tracking-[0.2em] text-stone-300">
                  Đồng bộ •{" "}
                </span>
                <select
                  value={preferences?.selectedModelId || ""}
                  ...
                  className="appearance-none bg-transparent outline-none text-[9px] font-bold uppercase tracking-[0.2em] text-stone-400 hover:text-stone-600 transition-colors cursor-pointer"
                  title="Chọn AI Model (Nhấn để đổi)"
                >
                  {AVAILABLE_MODELS.filter(m => m.tags?.includes('reasoning')).map((model) => (
                    <option key={model.id} value={model.id}>
                      {model.name}{" "}
                      {model.supportsVision === false ? "(No Vision)" : ""}
                    </option>
                  ))}
                </select>
                <ChevronDown className="w-3 h-3 text-stone-300 pointer-events-none -ml-0.5" />
              </div>
            </div>
          </div>
              ...
              <div className="h-full flex flex-col items-center justify-center text-center space-y-6 max-w-md mx-auto">
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-stone-500 text-sm leading-relaxed">
                    Hãy chọn một cuộc hội thoại cũ hoặc bắt đầu một cuộc hội
                    thoại mới để GemAgent hỗ trợ bạn thiết kế Menu, Food Styling
                    hoặc Content Social Media.
                  </p>
                </div>
                <button
                  onClick={startNewChat}
                            ...
                            <div className="space-y-1">
                              ...
                              <p className="text-lg font-display font-bold text-stone-900 text-green-600">
                                {new Intl.NumberFormat("vi-VN", {
                                  style: "currency",
                                  currency: "VND",
                                }).format(msg.recipe.recommendedPrice || 0)}
                              </p>
                            </div>
                          </div>
                              ...
                              <table className="w-full text-[10px] text-left">
                                <thead>
                                  <tr className="text-stone-400 border-b border-stone-50">
                                    <th className="pb-2 font-bold uppercase tracking-tighter">
                                      Nguyên liệu
                                    </th>
                                    <th className="pb-2 font-bold uppercase tracking-tighter text-right">
                                      Lượng
                                    </th>
                                    <th className="pb-2 font-bold uppercase tracking-tighter text-right text-stone-900">
                                      Ước tính (VNĐ)
                                    </th>
                                  </tr>
                                </thead>
                                <tbody className="divide-y divide-stone-50">
                                  {Array.isArray(msg.recipe.ingredients) &&
                                    msg.recipe.ingredients.map(
                                      (ing: any, idx: number) => (
                                        <tr
                                          key={idx}
                                        ...
                                        >
                                          ...
                                          <td className="py-2 text-stone-500 text-right">
                                            {ing.amount} {ing.unit}
                                          </td>
                        ...
                        <div className="flex flex-wrap gap-2 mt-2">
                          {(msg as any).attachments.map(
                            (att: Attachment, i: number) => (
                              <div
                                key={i}
                                className="relative rounded-xl overflow-hidden border border-stone-200 bg-white p-1 shadow-sm"
                              >
                                {att.type === "image" ? (
                                  <img
                                  ...
                                  <div className="w-24 h-24 flex flex-col items-center justify-center bg-stone-50 rounded-lg gap-1">
                                    ...
                                    <span className="text-[8px] font-bold text-stone-400 uppercase tracking-tighter truncate w-full px-1 text-center">
                                      {att.name}
                                    </span>
                                  </div>
                                ) : (
                          ...
                          >
                            ...
                            <ChevronDown
                              className={cn(
                                ...
                                showMonologue[msg.id] && "rotate-180"
                              )}
                            />
                          </button>
                          <AnimatePresence>
          ...
          <div className="max-w-4xl mx-auto space-y-3">
                      ...
                      <div className="w-16 h-16 rounded-xl overflow-hidden border border-stone-200 bg-white">
                        {att.type === "image" ? (
                          ...
                          <div className="w-full h-full flex flex-col items-center justify-center gap-1 bg-stone-100">
                            ...
                            <span className="text-[6px] font-bold text-stone-500 truncate w-full px-0.5 text-center">
                              {att.name}
                            </span>
                          </div>
                        )}
            ...
            <div className="relative flex items-center gap-3">
              ...
              <input
                ...
                placeholder={
                  activeConversationId
                    ? "Hỏi GemAgent về thiết kế, styling..."
            ...
            <p className="text-[9px] text-center text-stone-400 font-bold uppercase tracking-[0.2em]">
              ...
              {AVAILABLE_MODELS.find(
                (m) => m.id === preferences.selectedModelId
              )?.name || preferences.selectedModelId}{" "}
